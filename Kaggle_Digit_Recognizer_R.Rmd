...
library(openxlsx)
END_DATE <- Sys.Date()
FILE_PATH <- paste0(dirname(rstudioapi::getActiveDocumentContext()$path), "/Output/Submission_",format(END_DATE,"%Y%m"),".csv")
...
#Solutons
```{r, eval=FALSE }
# Function to convert matrix to vector of column indices with 1
convert_to_index_vector <- function(row) {
  which(row == 1)
}

# Apply the function to each row
index_vector <- apply(test_labels, 1, convert_to_index_vector)
print(index_vector)
ImageId=as.data.frame(1:nrow(index_vector))
submission<-as.data.frame(cbind(ImageId,index_vector))
